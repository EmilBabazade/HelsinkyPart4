...
describe('Author with most likes', () => {
	test(' of empty list is undefined', () => {
		expect(listHelper.mostLikes(lists.withNoBlogs)).toEqual(undefined)
	})

	test(' of list with many blogs is selected correctly', () => {
		expect(listHelper.mostLikes(lists.withManyBlogs)).toEqual({
			author: 'Edsger W. Dijkstra',
			likes: 46,
		})
	})
	...
	test(' of list with one blog is that author with 1 blog', () => {
		expect(listHelper.mostLikes(lists.withOneBLog)).toEqual({
			author: 'Edsger W. Dijkstra',
			likes: 5,
		})
	})
